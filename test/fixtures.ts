...
export async function deployRouterMockEnvironment() {
    const wasabiPoolsAndRouterFixture = await deployWasabiPoolsAndRouter();
    const {wasabiRouter, wasabiLongPool, wasabiShortPool, wethVault, ppgVault, uPPG, weth, orderSigner, orderExecutor, user1, user2, publicClient, wethAddress, debtController} = wasabiPoolsAndRouterFixture;
    ...
    const swappedAmount = shortDownPayment * initialPrice / priceDenominator;
    ...
    const longOpenPositionRequest: OpenPositionRequest = {
        ...
        minTargetAmount: longTotalSize * initialPrice / priceDenominator,
        expiration: BigInt(await time.latest()) + 86400n,
        fee: longFee,
        functionCallDataList: 
    ...
    const shortOpenPositionRequest: OpenPositionRequest = {
        ...
        minTargetAmount: shortPrincipal * initialPrice / priceDenominator,
        expiration: BigInt(await time.latest()) + 86400n,
        fee: shortFee,
        functionCallDataList: 
